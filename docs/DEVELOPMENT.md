...
- 搜索中的 GPIO、连接器和短英文目录词使用结构化边界或 token 词首匹配；用户自行填写的别名与备注保持普通子串搜索，二者必须分别回归。
...
- `outputs/` 只保存当前可覆盖的网页版候选；`releases/vX.Y.Z/` 不可覆盖。含同名 Git 标签且没有 `REJECTED.md` 的目录才是正式归档；带 `REJECTED.md` 的目录仅保留验收审计，禁止分发。
- beta 候选可以提交，但不创建正式标签。正式归档验证通过后再创建对应 `vX.Y.Z` 标签；验收失败的归档必须标记拒绝并提升版本，不能删除、覆盖或打标签。
