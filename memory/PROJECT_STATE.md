...
- 候选功能：两颗芯片各自的天猛星 PM-64 模板，含 17 项默认功能、56 个 GPIO 排针映射、NRST、板载资源、电气限制和 SPI Flash/H8 LCD-OLED 共享 SPI1 展示。
- 界面：Windows x64 桌面优先，固定夜间主题；打印报告白底。

## 兼容基线

- Electron 内部地址：`app://mspm0/index.html`。
- 本地存储：workspace v5、project v4，工程可选保存 `boardPresetId`。
- 用户数据保存在当前电脑的 Electron 用户数据目录，不随程序移动。
- 依赖版本：Electron `31.7.7`、electron-builder `24.13.3`。

## 最近验证

- `1.3.0-beta.1` 数据校验、板卡校验、JavaScript 语法检查和离线 HTML 构建通过。
