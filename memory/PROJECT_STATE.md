...
- 顶部工程与操作按钮保持单排紧凑布局，标题下方不再显示说明小字；免责声明和数据来源仍保留在页面底部与关于窗口中。
...
- 未运行本轮 Electron 全量烟雾测试；当前候选等待用户目视检查外设详情栏与画布压缩效果。

## 工作区

- 正式工作区：`<WORKSPACE>`。
- 候选功能检查点：`1.3.0-beta.12` 源码检查点，不创建正式标签。
- Git 远程 `origin` 已配置为 `https://github.com/ww2186874-cyber/TI_Planner.git`；全部正式标签已推送，当前 `1.3.0-beta.12` 候选提交保留在本地 `main`，尚未推送至 `origin/main`。
- `outputs/` 只保留当前 `1.3.0-beta.12` HTML 和 `.gitkeep`；旧候选及已有正式版本副本已清理，正式版本仍完整保存在 `releases/`。
