...
- 候选功能：两颗芯片各自的天猛星 PM-64 模板，含 5 项固定时钟默认功能、8 项可切换板载资源、56 个 GPIO 排针映射、板载电气限制和 SPI Flash/H8 LCD-OLED 共享 SPI1 展示。
...
- `1.3.0-beta.2` 数据校验、板卡资源映射校验、JavaScript 语法检查、烟雾脚本语法检查和离线 HTML 构建通过；尚未运行 Electron 全量烟雾测试。
...
- `outputs/` 只保留当前 `1.3.0-beta.2` HTML 和 `.gitkeep`；旧候选及已有正式版本副本已清理，正式版本仍完整保存在 `releases/`。
