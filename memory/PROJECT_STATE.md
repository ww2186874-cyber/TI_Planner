...
- 当前正式版本：`1.1.0`
- 当前候选版本：`1.2.0-beta.1`；已重新生成包含 GPIO 精确搜索修复的离线 HTML，快速启动文件夹版仍是修复前候选，等待 HTML 确认后重建
- 最新 Git 标签：`v1.1.0`
...
- 最近完整验证：`1.2.0-beta.1` 文件夹版正常启动；两颗芯片的 RHB/RGZ 封装列表、首尾脚、32/48 引脚数、真实标签输入、正常关闭恢复、v3/v4 导入、打印报告和保存桥接通过；搜索修复在真实 Electron 页面验证 `PB1` 只命中 `PB1`，不再命中 `PB10/PB11/PB12`；全部 LQFP/VQFN 数据校验、旧封装无回归比对及提取结果可复现
...
工作区预期状态：Git干净；正式版本不可覆盖；`outputs/` 保存 `1.2.0-beta.1` 搜索修复后候选 HTML、修复前文件夹版和既有候选产物；未生成本候选的便携 EXE或正式发布归档。
