...
- 当前正式版本：`1.1.0`
- 当前候选版本：`1.2.0-beta.1`，已生成离线 HTML，等待用户确认
- 最新 Git 标签：`v1.1.0`
...
- 最近完整验证：`1.2.0-beta.1` 的两颗芯片和全部 LQFP/VQFN 数据校验、旧封装无回归比对、提取结果可复现及离线 HTML 构建通过；文件夹版和 Electron 烟雾测试等待 HTML 确认后执行
...
工作区预期状态：Git干净；正式版本不可覆盖；`outputs/` 仅保存 `1.2.0-beta.1` 候选 HTML 和既有候选产物；未生成本候选的文件夹版、便携 EXE或正式发布归档。
