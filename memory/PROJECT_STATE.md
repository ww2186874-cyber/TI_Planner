...
- `1.3.0-beta.14` 将工程选择组在 1400 px 及以上宽屏中固定到顶部栏水平中心；较窄窗口恢复正常流式排列，标题仍靠左、操作按钮仍靠右。
...
- 正式工作区：`<WORKSPACE>`。
- 候选功能检查点：`1.3.0-beta.14` 源码检查点，不创建正式标签。
- Git 远程 `origin` 已配置为 `https://github.com/ww2186874-cyber/TI_Planner.git`；全部正式标签已推送，当前 `1.3.0-beta.14` 候选提交保留在本地 `main`，尚未推送至 `origin/main`。
- `outputs/` 只保留当前 `1.3.0-beta.14` HTML 和 `.gitkeep`；旧候选及已有正式版本副本已清理，正式版本仍完整保存在 `releases/`。
