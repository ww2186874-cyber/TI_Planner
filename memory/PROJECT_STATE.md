...
- `1.3.0-beta.4` 数据校验、板卡资源映射校验、JavaScript 语法检查、烟雾脚本语法检查和离线 HTML 构建通过；尚未运行 Electron 全量烟雾测试。
...
- 候选功能检查点：`1.3.0-beta.4` 源码检查点，不创建正式标签。
- Git 远程 `origin` 已配置为 `https://github.com/ww2186874-cyber/TI_Planner.git`，`main` 和全部正式标签已推送并与远程一致。
- `outputs/` 只保留当前 `1.3.0-beta.4` HTML 和 `.gitkeep`；旧候选及已有正式版本副本已清理，正式版本仍完整保存在 `releases/`。
